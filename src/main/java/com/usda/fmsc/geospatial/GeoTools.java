...
public class GeoTools {
    ...
    public static Position getMidPioint(List<Position> positions) {
    ...
    public static PositionLegacy getGeoMidPointLegacy(PositionLegacy position1, PositionLegacy position2) {
        ArrayList<PositionLegacy> arr = new ArrayList<>();

        arr.add(position1);
        arr.add(position2);

        return getGeoMidPointLegacy(arr);
    }

    public static PositionLegacy getGeoMidPointLegacy(List<PositionLegacy> positions) {
        Double x, y, q;
        x = y = q = 0.0;
        ...
        Double lat, lon;
        for (PositionLegacy p : positions) {
        ...
        return new PositionLegacy(new Latitude(lat * 180.0 / Math.PI), new Longitude(lon * 180.0 / Math.PI));
    }

    public static GeoPosition getMidPiointLegacy(List<GeoPosition> positions) {
        if (positions == null || positions.size() < 1) {
            throw new RuntimeException("No positions to average.");
        }
